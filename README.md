...
**Zone C가 핵심 차별점.** LLM이 어떤 도구를 왜 선택했는지 실시간으로 보인다. 질문을 바꿀 때마다 다른 도구 조합이 보이므로
...
lift, correlation, p-value를 LLM이 생성하면 **통계적 근거 없는 숫자**가 된다. 코드가 SQL 집계로 계산하고, LLM은 "어떤 속성 쌍을 검사할지"만 판단한다. 계산 결과의 수치는 evidence로 DB에 저장되어 재현·검증이 가능하다.
