...
역직구 뷰티 상품의 비정형 속성을 구조화하고, Neo4j 인과 체인 위에 실 데이터를 배치하여, LLM이 자율적으로 데이터 소스를 조합해 셀러에게 인사이트를 제공하는 것이 목적

![대시보드 전체 화면](docs/images/image.png)
...
크로스보더 이커머스(역직구)에서 화장품/뷰티 화주의 실 주문 데이터는 운영에만 쓰이고 있다. 
국가별·채널별·속성별로 풍부한 시장 인사이트가 묻혀 있는데, 이를 꺼내려면 **비정형 상품명에서 구조화된 속성을 추출**해야 한다.
...
검토했으나 채택하지 않은 대안:
- **Tool Use만** → 구조는 해결되지만, 새 카테고리 추가 시 Tool description 수정 필요. Few-Shot은 검증된 임베딩 데이터만 추가하면 되므로 코드 변경 없음(flywheel).
...
- **Multi-Turn 자기 검증** → 외부 피드백 없는 self-correction은 효과 제한적. 환각은 규칙 기반 검증으로, 누락은 Eval로 커버. 비용 2배인데 품질 향상이 유의미 하지 않다고 느껴 제거.

### 접근 2: Neo4j 온톨로지 — 인과 체인이 "왜?"에 해당하는 인관관계 설명에 답할 수 있도록
...
검토했으나 채택하지 않은 대안:
- **PostgreSQL JSON** → 5개 테이블 JOIN으로 인과 체인 표현 가능하지만 복잡하며, 그 맥락을 주입하기 바람직한 아키는 아님 새 관계 추가 시 JOIN 변경.
- **SOLD_IN에 주문량 누적** → Neo4j에 집계 데이터를 넣으면 월별 시계열 불가. **Neo4j = "왜?"(인과), PostgreSQL = "얼마나?"(집계)** 역할 분리를 원칙으로

### 접근 3: LLM 오케스트레이터 — 도구 자율 선택

셀러의 질문에 따라 Neo4j(인과) + PostgreSQL(집계)을 LLM이 자율적으로 조합한다. while-loop ReAct 패턴으로 Anthropic SDK 직접 구현하여 LLM의 사고 프로세스를 쫒기 유용한 구조로 만들고자 함
...
flowchart TB
    ...
    subgraph KG["KG Server - Neo4j"]
        K1["인과 체인"]
        K2["성분 시너지"]
        K3["상품 그래프"]
    end

    subgraph OD["Order Server - PostgreSQL"]
        D1["월별 트렌드"]
        D2["속성 히트맵"]
        D3["비율 집계"]
    end

    KG --> T["tool_call_traces<br/>판단 근거 + 비용 추적"]
    OD --> T

    subgraph EX["속성 추출 파이프라인"]
        direction LR
        E1["상품명"] --> E2["ChromaDB<br/>벡터 검색"]
        E2 --> E3["LLM<br/>Tool Use + Few-Shot"]
        E3 --> E4["규칙 기반 검증"]
        E4 --> E5["PostgreSQL 적재<br/>Neo4j graph_sync"]
    end
```
...
**[구축된 온톨로지 예시]**
<img width="1435" height="485" alt="구축된 상품 마스터의 온톨로지 샘플" src="https://github.com/user-attachments/assets/25adad34-ac21-457c-be79-c5fec625dedf" />

<img width="1555" height="748" alt="일본 상품 온톨로지 샘플" src="https://github.com/user-attachments/assets/771f0af0-2822-40e2-8b38-ecd62d788c99" />
...
[적재 + graph_sync]
    ...
    errors 있음 → extractions INSERT만 (gr화aph_sync 안 함)
```

**[상품명 임베딩 시각화]**
![상품명 임베딩 공간 시각화](docs/images/image-3.png)
...
2. **MVP에서 운영 시나리오 없음** — MVP 단계에선 "이건 자동 승인, 저건 검수"를 분류할 필요가 없이 초기 검수를 필요로 온톨로지를 구축하는 것이 최우선이라는 판단
3. **validator 판단으로 충분** — errors 없으면 온톨로지에 편입, 있으면 안 함. 이 이분법으로 우선 MVP에서 구현하고자 함

### 설계 결정: 추출 시 원본 키워드 보존 원칙

"톤업"을 "브라이트닝"으로, "노세범"을 "피지조절"로 동의어 변환하면 `get_attribute_trend("톤업")`으로 검색 시 0건이 반환된다. 
추출 단계에서는 상품명의 키워드를 그대로 저장하고, 상위 카테고리 매핑(톤업 ∈ 브라이트닝)은 집계 단계에서 별도 처리한다. 정보 손실 방지 + LLM 추출과의 형식 일관성.

각 속성의 위계를 잡는 것은 그 다음 스텝이라고 판단
...
수동으로 300줄짜리 JSON 스키마를 쓰는 대신, 각 도구의 입력을 Pydantic `BaseModel`로 정의하고 `@tool` 데코레이터를 부착한다. 
`model_json_schema()`가 Anthropic Tool의 `input_schema`를 자동 생성하고, 함수의 docstring이 `description`이 된다.
...
도구를 추가할 때 서버 파일 1곳만 수정. 오케스트레이터 코드 변경 불필요(단일 수정 지점 원칙). 
런타임에 Pydantic이 입력을 검증하여 LLM이 잘못된 파라미터를 보내면 조기 감지.
...
TraceLogger가 "이 도구가 어느 서버에 속하는지"를 알아야 하는데, 도구 목록을 하드코딩하면 도구 추가 시 TraceLogger를 수정해야 한다. 

대신 오케스트레이터가 서버별 도구를 등록할 때 매핑을 자동 생성하여 TraceLogger에 주입한다. 
TraceLogger는 매핑만 받으면 되고, 도구 구성을 알 필요 없다(관심사 분리).

### ReAct while-loop: text block 캡처

LLM 응답에서 text block(사고 과정)과 tool_use block(도구 선택)을 분리 캡처한다. 
text block이 trace의 `reasoning` 필드가 되어, 프론트엔드에서 "왜 이 도구를 선택했는지" 의사결정 흐름도를 렌더링할 수 있다.
...
추후 임계치 이상의 동일 질문에 대해 LLM 호출 횟수를 최적화 하는 것도 고려
...
**Zone C.** LLM이 어떤 도구를 왜 선택했는지 실시간으로 보인다. 질문을 바꿀 때마다 다른 도구 조합이 보이므로

* 사용자에게 LLM 판단 시각화 및 신뢰성 및 튜닝 point 도출할 수 있도록
...
**[질의에 따라 달라지는 LLM 의 Tool 사용]**
<img width="2880" height="730" alt="상황에 따른 Tool 사용" src="https://github.com/user-attachments/assets/9b7ec3e1-d81b-48fd-89ac-42431e4d9ad5" />
...
AnalystAgent는 Neo4j의 시드 인과 체인(기후→피부고민→기능)을 읽어서 "왜?"에 답한다. 
하지만 시드 온톨로지는 도메인 전문가가 사전에 정의한 구조이므로, **데이터에서 새롭게 발생하는 패턴을 반영하지 못한다.** 
예를 들어 "비건과 무기자차가 왜 함께 많이 나타나는가?"라는 질문에 AnalystAgent는 두 속성을 각각 따로 설명할 뿐, 동시 출현이 독립 기대치보다 높다는 **통계적 관계**는 시드에 없으므로 답할 수 없다.

이것은 시드 온톨로지 설계의 문제가 아니라 구조적 한계다. 시드는 "기후→피부고민→기능"이라는 **도메인 지식 기반의 인과 체인**을 담고 있고, 이 인과 체인은 데이터와 무관하게 참이다. 

하지만 "비건 속성과 무기자차 속성이 독립 기대치보다 1.54배 많이 동시 출현한다"는 **데이터에서 관찰된 통계적 사실**이며, 이것은 시드에 사전 정의할 수 없다.

이렇듯 사용할수록 지식체계가 확장 되어가는 구조를 설계하고자 함

### PatternScout: 데이터에서 통계적 패턴을 찾아 온톨로지를 확장

PatternScout는 AnalystAgent와 **목적이 다른** 별도 에이전트다. 

AnalystAgent가 "사용자 질문에 답하기"라면, PatternScout는 "주문 데이터에서 통계적으로 유의미한 속성 간 패턴을 탐지"하는 것이 목적이다.

**핵심 사상**: 이 시스템이 찾는 것은 "인과"가 아니라 **"통계적 공존 패턴"**이다. 
인과 검증은 프로덕션 A/B 테스트의 영역이고, PatternScout가 제안하는 것은 상관(correlation)이다. 
시드 온톨로지의 기후→피부고민→수요는 도메인 지식에 기반한 인과이지만, PatternScout가 발견하는 것은 데이터에서 관찰된 통계적 관계다.
...
"자유롭게 가설을 세우세요"는 LLM이 뭘 해야 할지 몰라서 안전한 조회만 하고 끝난다. 
실제로 테스트하면 LLM은 위험 회피적으로 행동하여 `get_attribute_trend` 같은 안전한 읽기 도구만 반복 호출하고, `propose_relationship` 같은 쓰기 도구는 호출하지 않는다. 
대신 명시적 탐색 절차를 system prompt로 제공한다:
...
**LLM의 역할과 코드의 역할을 명확히 분리한 이유**: LLM이 "lift가 1.54입니다"라고 생성하면, 이 숫자는 어디서 왔는지 검증할 수 없다. 
LLM의 내부 가중치에서 나온 숫자이지, SQL 집계 결과가 아니다. 반면 `compute_cooccurrence_lift()`가 반환한 1.54는 `COUNT(*) FILTER WHERE ... / total`의 결과이므로 재현 가능하고, evidence에 SQL 결과를 그대로 저장하여 누구든 검증할 수 있다.
...
lift, correlation, p-value를 LLM이 생성하면 **통계적 근거 없는 숫자**가 된다. 
코드가 SQL 집계로 계산하고, LLM은 "어떤 속성 쌍을 검사할지"만 판단한다. 
계산 결과의 수치는 evidence로 DB에 저장되어 재현·검증이 가능하다.
...
**3. 매 턴 실행의 비용 비효율** — MVP에서 PatternScout는 매 AnalystAgent 턴 후 자동 실행된다. 같은 국가+카테고리에 대해 반복 탐색하면 이미 발견한 패턴을 다시 검사하게 된다. 프로덕션에서는 일 배치/이벤트 트리거로 전환해야 한다. MVP에서 매 턴 실행하는 이유는 결과를 즉시 확인할 수 있도록 하기 위함.

---

## Eval 프레임워크: Knowledge Growth 측정

![Knowledge Growth 대시보드](docs/images/image-2.png)
...
이 수치를 증가 시켜 실제 온톨로지가 확장되어 답변의 지식 그래프가 실제로
넓어지고 있는지를 확인하고자 함

### Before/After 답변 비교 — 숫자보다 강한 증거

discovered_usage_rate 숫자보다, **두 답변을 나란히 읽는 것**이 초기에는 추적하기 쉬울 것 같아,
 같은 질문의 승인 전/후 답변을 자동으로 찾아서 대시보드에서 비교할 수 있도록 구현


* 이들의 수치값과 EVAL의 구조를 잡는 것이 훨씬 많은량의 추후 직업을 요구할 것 같다
...
| LLM-as-judge (Claude가 두 답변을 비교) | LLM이 LLM을 평가하는 순환. 사람이 직접 읽는 게 더 신뢰 |
| Trace step 수 비교 | Before/After 모두 3 steps로 차이 없음. 의미 없음 |
| 답변 길이 비교 | 길다고 좋은 게 아님 |
| 속성 추출 F1 | 현재 범위(한국어, 4브랜드, 5카테고리)에서 난이도 낮아 유의미한 차별화 어려움 |
...
**6. 속성 추출 Eval의 실무 격차** — 현재 Eval은 PatternScout의 온톨로지 확장이 답변 품질을 개선하는가에 초점을 맞추고 있다. 프로덕션 환경에서는 이에 더해 **속성 추출 자체의 품질 측정**이 필수적이다. 구체적으로:
- **다국어 대응 정확도**: 현재는 한국어 상품명만 처리하지만, 역직구의 실 데이터는 일본어·영어·중국어 상품명이 혼재. 언어별 추출 정확도(F1)를 분리 측정하고, 언어 감지 → 언어별 Few-Shot 예시 선택의 파이프라인이 필요.
- **추출 모델 성능 비교**: Haiku vs Sonnet의 추출 정확도와 비용 트레이드오프를 정량화하는 Eval이 없다. Gold holdout 기반의 필드별 F1 측정 + 모델별 비교 파이프라인을 구축하면, "배치는 Haiku로 충분, 검수용은 Sonnet"이라는 라우팅 근거를 데이터로 만들 수 있다.
- **스키마 드리프트 모니터링**: 추출 결과의 키 이름·값 형식이 시간이 지나도 일관되는지를 연속 측정하는 체계. Tool Use가 구조를 강제하지만, 프롬프트 변경이나 모델 업데이트 시 미세한 형식 차이가 발생할 수 있으며 이를 자동 감지해야 한다.

이 격차를 인지하고 있으며, 현재 시스템의 Eval 인프라(스냅샷 비교, 4축 측정, ADR 기반 지표 관리)가 이러한 확장을 수용할 수 있는 구조로 설계되어 있다.

### 향후 계획

- **속성 추출 Eval 고도화** — Gold holdout 기반 필드별 F1 + 모델별 비용-정확도 비교 + 다국어 대응
