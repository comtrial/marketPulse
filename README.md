...
14개의 아키텍처 결정을 [`docs/adr/`](docs/adr/)에 기록했다. 각 결정은 맥락, 검토한 대안, 선택 근거, 결과를 포함한다.
...
**1. 시뮬레이션 데이터** — 실제 Shopee/Qoo10/Cafe24 API 연동이 아닌 `generate_orders.py`로 생성한 1,000건의 시뮬레이션 데이터. 의도적으로 6개 패턴(A~F)을 심어서 시스템 검증이 가능하지만, 실 데이터의 노이즈와 규모는 반영하지 못함. MCP 서버 구조이므로 시뮬레이션 서버를 실 API 서버로 교체하면 나머지 코드 변경 없이 동작.

**2. Eval 프레임워크 미구현** — 추출 정확도(F1), 도구 선택 정확도, 답변 품질을 체계적으로 측정하는 Eval이 아직 없음. ANSWER_SHEET에 15개 스토리와 검증 기준을 정의해뒀으나 자동화된 평가 파이프라인은 Phase 2에서 구현 예정. TRAJECT-Bench(ICLR 2026)의 trajectory-level evaluation과 팔란티어 AIP Evals의 intermediate step evaluation 접근법을 참고하여, 도구 선택·파라미터·순서·출력 활용·사고-행동 일치를 평가하는 5-dimension trajectory eval + Neo4j 온톨로지를 검증 기준으로 활용하는 ontology-grounded eval을 설계 중.

**3. 스트리밍 미지원** — 오케스트레이터가 전체 완료 후 응답. Step별 SSE 스트리밍을 구현하면 Zone C가 실시간으로 업데이트되는 UX가 가능하지만 현재 미구현.

**4. 4개 브랜드 한정** — Neo4j에 시드된 브랜드가 이니스프리, 토리든, 라운드랩, 스킨푸드 4개뿐. 실제 역직구에는 수백 개 브랜드가 존재하며, 브랜드 자동 등록 메커니즘이 필요.

### Phase 2 계획

- **Eval 프레임워크** — 5-dimension trajectory eval + ontology-grounded eval
- **PatternScoutAgent** — 블루오션 탐색, 셀러 갭 분석 도구 구현
- **자동 속성 승격** — additionalAttrs 빈도 카운팅 → 임계치 초과 시 정규 필드 승격
