...
Zone C는 LLM의 의사결정 과정을 보여주는 트레이스 패널이다. "LLM이 질문에 따라 다른 도구를 자동 선택한다"는 것을 한눈에 확인할 수 있어야 한다.
...
- **직관적 이해**: Think 블록이 시각적으로 강조되면 "LLM이 추론하고 있다"는 것이 즉시 전달된다.
- **운영 가시성**: Latency 워터폴, 토큰 그리드, 서버 분포 바가 시스템 상태를 투명하게 보여준다.
